...
class Logging(commands.Cog):
    ...
    async def setlogsetup(self, ctx):
        """Creates Astra-logging category and log channels with private permissions"""
        guild = ctx.guild
        ...
        category = discord.utils.get(guild.categories, name="Astra-logging")
        if not category:
            category = await guild.create_category("Astra-logging", overwrites=overwrites)
        ...
        await ctx.send("✅ Logging channels created privately under 'Astra-logging'.")

    @commands.command(name="removelogs")
    @commands.has_permissions(administrator=True)
    async def removelogs(self, ctx):
        """Removes Astra-logging channels and logging DB config"""
        guild = ctx.guild
        ...
        await ctx.send("🗑️ Logging channels and category 'Astra-logging' have been removed.")
