  ...
  async def antinuke(self, ctx, option: str = None):
    ...
    if option is None:
      ...
      if is_activated:
        ...
        setup_embed.description += "\n<:tick:1327829594954530896>| Checking Astra's role position for optimal configuration..."
        await setup_message.edit(embed=setup_embed)

        await asyncio.sleep(1)
        setup_embed.description += "\n<:tick:1327829594954530896> | Crafting and configuring the Astra Supreme role..."
        await setup_message.edit(embed=setup_embed)
        ...
        await asyncio.sleep(1)
        setup_embed.description += "\n<:tick:1327829594954530896>| Ensuring precise placement of the Astra Supreme role..."
        await setup_message.edit(embed=setup_embed)
        try:
        ...
        embed.set_author(name="Astra Antinuke", icon_url=self.bot.user.avatar.url)

        embed.set_footer(text="Successfully Enabled Antinuke for this server | Powered by Astra X Development™", icon_url=self.bot.user.avatar.url)
        embed.set_thumbnail(url=self.bot.user.avatar.url)
