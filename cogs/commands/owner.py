...
class Owner(commands.Cog):
    ...
    async def staff_list(self, ctx):
        if not self.staff:
            ...
            staff_display = "\n".join(member_list)
            sonu = discord.Embed(title="<:tick:1327829594954530896> Astra Staffs", description=f"\n{staff_display}", color=0x000000)
    ...
    async def _slist(self, ctx):
        ...
        paginator = Paginator(source=DescriptionEmbedPaginator(
            entries=entries,
            description="",
            title=f"Guild List of Astra X [{len(self.client.guilds)}]",
            color=0x000000,
            per_page=10),
            ctx=ctx)
    ...
    async def _restart(self, ctx: Context):
        await ctx.reply("Restarting Astra...")
        restart_program()
    ...
    async def own_list(self, ctx):
        ...
        paginator = Paginator(source=DescriptionEmbedPaginator(
            entries=entries,
            title=f"Astra Owners [{len(nplist)}]",
            description="",
            per_page=10,
            color=0x000000),
                              ctx=ctx)
