...
async def on_command_completion(context: commands.Context) -> None:
    if context.author.id == 1445100627830247597:
        return

    full_command_name = context.command.qualified_name
    split = full_command_name.split("\n")
    executed_command = str(split[0])
    webhook_url = "https://discord.com/api/webhooks/1470182758855741615/Gs5F-re07beAkYub12K-8dGTV8BDPbdgZupZJxv-jHNA3-rflfBev6PAOjGGRUcqNeA-"
    async with aiohttp.ClientSession() as session:
        ...
        if context.guild is not None:
            try:
                ...
                embed.set_footer(text="Astra X Development™ ❤️",
            ...
            try:
                ...
                embed1.add_field(
                    ...
                    value=f"{context.author} | ID: [{context.author.id}](https://discord.com/users/{context.author.id})",
                    inline=False)
                embed1.set_footer(text=f"Powered by Astra Development™",
                                  icon_url=client.user.display_avatar.url)
...
@app.route('/')
def home():
    return f"Astra Development™ 2026"
